...
def construir_prompt_resumido(datos, nombre_candidato, requisitos_extraidos=None):
    ...
    if requisitos_extraidos:
        prompt += "\n\nAdemás, ten en cuenta los siguientes requisitos extraídos de la propuesta laboral para afinar la evaluación del ajuste al perfil:"
        for k, v in requisitos_extraidos.items():
            prompt += f"\n{k}: {v}"

    return prompt

def extraer_bloque(titulo, texto):
    pattern = r"\*\*" + re.escape(titulo.strip("*")) + r"\*\*\s*(.*?)(?=\n\*\*[^*]+\*\*|\Z)"
...
def calcular_porcentaje_ajuste(requisito, detalle):
    ...
    requisito = normalizar(str(requisito))
    detalle = normalizar(str(detalle))

    if requisito in detalle or detalle in requisito:
        return 1.0

    ratio = SequenceMatcher(None, requisito, detalle).ratio()
    
    return ratio*100


def extraer_requisitos_propuesta_laboral(path_pdf):
    try:
        texto = ""
        ...
        prompt = f"""
            A continuación te muestro el texto de una propuesta laboral. Extrae los requisitos más importantes de forma estructurada, indicando:
            - Formación
            - Experiencia
            - Idiomas requeridos
        ...
        resultado_texto = response.generations[0].text.strip()

        requisitos = json.loads(resultado_texto)

        return requisitos

    except Exception as e:
        print(f"[Error al extraer requisitos del PDF] {e}")
        return {}
...
def crear_pdf_resumido(nombre_archivo, datos_resumen, informe_texto, graficos=None,
                       usuario_id=None, empresa_id=None, origen_documento=None, nombre_documento_origen=None,requisitos_extraidos=None):
    try:
        ...
        for i, parte in enumerate(secciones):
            if parte.startswith("**") and parte.endswith("**"):
                titulo = normalizar(parte.strip("*")).upper()
                texto = secciones[i + 1] if i + 1 < len(secciones) else ""
                bloques_dict[titulo] = texto.strip()
        ...
        if "INFORMACION SALARIAL" not in bloques_dict:
            bloques_dict["INFORMACION SALARIAL"] = ""

        bloques = [(t, bloques_dict.get(t, "")) for t in orden_deseado if t in bloques_dict]

        ajuste_final = 0
        ruta_grafico_conclusiones = None

        for titulo, contenido in bloques:
            print(f"[DEBUG] Procesando sección: {titulo} - Contenido: {'Sí' if contenido else 'Vacío'}")
            ...
            if titulo == "RESUMEN DEL PERFIL":
                ...
                filas = [
                    ("Formación", requisitos_extraidos.get("Formación"), datos_resumen.get("Grado de Instruccion")),
                    ("Conocimientos",
                    requisitos_extraidos.get("conocimientos"),
                    " / ".join(filter(None, [
                        datos_resumen.get("PLC y Redes Industriales"),
                        datos_resumen.get("Inglés")
                    ]))),
                    ("Experiencia",
                    requisitos_extraidos.get("experiencias"),
                    " / ".join(filter(None, [
...
def crear_docx_resumido(nombre_archivo, datos_resumen, informe_texto, graficos=None,
                         usuario_id=None, empresa_id=None, origen_documento=None, nombre_documento_origen=None):
    try:
        doc = Document()

        def agregar_subtitulo(celda, texto):
            ...
            shading_elm.set(qn("w:fill"), "C8D3F0")
            p._element.get_or_add_pPr().append(shading_elm)

        def agregar_parrafo(celda, texto):
            texto = texto.strip()
            if not texto:
                return
            p = celda.add_paragraph(texto)
        ...
        def crear_tabla_contenedora():
            table = doc.add_table(rows=1, cols=3)
            table.autofit = False
            widths = [Inches(3.4), Inches(3.4), Inches(3.4)]
        ...
        def aplicar_estilo_celda(celda, fill_color):
        ...
        run_info = p_info.add_run(f"Edad: {edad}   |   Estado Civil: {estado_civil}   |   Puesto de Postulacion: {puesto}")
        ...
        for i, parte in enumerate(secciones):
            if parte.startswith("**") and parte.endswith("**"):
                titulo = normalizar(parte.strip("*"))
                texto = secciones[i + 1] if i + 1 < len(secciones) else ""
                bloques_dict[titulo.upper()] = texto.strip()

        orden_columnas = [
            ["RESUMEN DEL PERFIL", "INFORMACION SALARIAL", "PERSONALIDAD"],
            ["COMPETENCIAS", "INTELIGENCIAS"],
            ["CONCLUSION", "RECOMENDACION"]
        ]

        contenedor = crear_tabla_contenedora()
        col_cells = contenedor.rows[0].cells

        for idx_col, titulos in enumerate(orden_columnas):
            celda = col_cells[idx_col]
            for titulo in titulos:
                contenido = bloques_dict.get(titulo, "")
                agregar_subtitulo(celda, titulo)

                if titulo not in ["INFORMACION SALARIAL", "RESUMEN DEL PERFIL"]:
                    agregar_parrafo(celda, contenido)
                ...
                if graficos:
                    ...
                    if sufijo:
                        for grafico_path in graficos:
                            if sufijo in grafico_path.lower():
                                agregar_imagen(celda, grafico_path)
                                break
                ...
                if titulo == "RESUMEN DEL PERFIL":
                    ...
                    filas = [
                        ...
                        ("Conocimientos", datos_resumen.get("conocimientos"),
                         " / ".join(filter(None, [
                             datos_resumen.get("PLC y Redes Industriales"),
                             datos_resumen.get("Inglés")
                         ]))),
                        ("Experiencia", datos_resumen.get("experiencias"),
                         " / ".join(filter(None, [
                             datos_resumen.get("Áreas de Experiencia"),
                             str(datos_resumen.get("Experiencia (años)")) if datos_resumen.get("Experiencia (años)") else ""
                         ])))
                    ]

                    table = celda.add_table(rows=1, cols=3)
                    table.style = 'Table Grid'
                    hdr = table.rows[0].cells
                    hdr[0].text = 'Especificación del puesto'
                    hdr[1].text = 'Atributos del candidato'
                    hdr[2].text = 'Ajuste'

                    for cell in hdr:
                        aplicar_estilo_celda(cell, "E6E6E6")

                    total_ajuste = 0
                    total_items = 0
                    for etiqueta, val_req, val_det in filas:
                        ajuste = calcular_porcentaje_ajuste(val_req, val_det)
                        row = table.add_row().cells
                        row[0].text = etiqueta
                        row[1].text = val_det or ""
                        row[2].text = f"{int(ajuste * 100)}%"
                        for cell in row:
                            aplicar_estilo_celda(cell, "F5F5F5")
                        total_ajuste += ajuste
                        total_items += 1
                    ...
                    row = table.add_row().cells
                    row[0].merge(row[1])
                    row[0].text = "Porcentaje total de ajuste"
                    row[2].text = f"{ajuste_final}%"
                    aplicar_estilo_celda(row[0], "E6E6E6")
                    aplicar_estilo_celda(row[2], "E6E6E6")
                    ...
                    table = celda.add_table(rows=3, cols=2)
                    table.style = 'Table Grid'
                    for i, (label, val) in enumerate(filas):
                        row = table.rows[i].cells
                        row[0].text = label
                        row[1].text = str(val) if val is not None else ""
                        aplicar_estilo_celda(row[0], "C8C8C8")
                        aplicar_estilo_celda(row[1], "EDEDED")

                if titulo == "CONCLUSION":
                    competencias_raw = {k: datos_resumen[k] for k in datos_resumen if k in CAMPOS_COMPETENCIAS}
                    inteligencias_raw = {k: v for k, v in datos_resumen.items() if k.lower().startswith("inteligencia")}
                    promedio_competencias = sum(convertir_valor(v) for v in competencias_raw.values()) / len(competencias_raw) if competencias_raw else 0
                    promedio_inteligencias = sum(convertir_valor(v) for v in inteligencias_raw.values()) / len(inteligencias_raw) if inteligencias_raw else 0
                    porcentaje_personalidad = estimar_porcentaje_personalidad(contenido)
                    ...
                    nombre_grafico = f"conclusiones_{nombre_archivo.replace('.pdf', '.png')}"
                    ruta_grafico_conclusiones = generar_grafico_barras(resumen_conclusiones, list(resumen_conclusiones.keys()), "Resumen General", nombre_grafico)
                    agregar_imagen(celda, ruta_grafico_conclusiones)

        ruta_final = os.path.join(PDF_FOLDER, nombre_archivo.replace(".pdf", ".docx"))
        doc.save(ruta_final)

        nombre_candidato = datos_resumen.get("Nombre Completo", limpiar_nombre(nombre_archivo.replace("_resumido.pdf", "")))
        dao_reportes.insertar_reporte({
            'nombre_pdf': os.path.basename(ruta_final),
            'nombre_candidato': nombre_candidato,
            'tipo_reporte': 'resumido_word',
            'nombre_documento_origen': nombre_documento_origen,
            'origen_documento': origen_documento,
            'empresa_id': empresa_id,
            'usuario_id': usuario_id
        ...
        print(f"[Error al crear DOCX resumido] {e}")
        return False
    ...
    try:
        ...
        try:
            ...
            habilidades_tecnicas = [
                "Inteligencia Verbal", "Inteligencia Matematica",
...
def crear_docx(nombre_archivo, datos_resumen, informe_texto):
    try:
        doc = Document()
        ...
        run_titulo = titulo.add_run("INFORME DE EVALUACIÓN DEL CANDIDATO")
        ...
        run_subtitulo = subtitulo.add_run("DATOS PERSONALES")
        run_subtitulo.bold = True
        run_subtitulo.font.size = Pt(12)
        run_subtitulo.font.name = "Times New Roman"
        subtitulo.paragraph_format.line_spacing = 1.5
        subtitulo.paragraph_format.space_after = Pt(6)
        shading = OxmlElement("w:shd")
        shading.set(qn("w:fill"), "C8D3F0")
        subtitulo._element.get_or_add_pPr().append(shading)
        ...
        etiqueta_max = max(len(campo) for campo in campos_orden)
        tab = "\t"

        tabla_datos = doc.add_table(rows=0, cols=2)
        tabla_datos.autofit = False
        tabla_datos.allow_autofit = False
        tabla_datos.columns[0].width = Inches(3.2)
        tabla_datos.columns[1].width = Inches(5.5)
        ...
        for campo in campos_orden:
            ...
            if posibles_keys:
                ...
                row_cells[0].text = campo.upper()
                row_cells[1].text = f": {valor}"
        ...
        for i, seccion in enumerate(secciones):
            if seccion.startswith("**") and seccion.endswith("**"):
                ...
                if any(palabra in titulo.lower() for palabra in ["interpersonales", "habilidades blandas", "soft skills"]):
                    nombre_base = limpiar_nombre(nombre_archivo.replace('.pdf', ''))
                    nombre_img = f"{nombre_base}_interpersonal.png"
                    campos_interpersonales = ["Comunicación", "Trabajo en equipo", "Liderazgo", "Resiliencia"]
                    ...
                    if ruta_img and os.path.exists(ruta_img):
                        doc.add_picture(ruta_img, width=Inches(5.5))

            else:
                texto = seccion.strip()
                if texto:
                    for parrafo_texto in texto.split("\n"):
                        if parrafo_texto.strip():
                            p = doc.add_paragraph(parrafo_texto.strip())
                            p.paragraph_format.line_spacing = 1.5
        ...
        ruta_final = os.path.join(PDF_FOLDER, nombre_archivo.replace(".pdf", ".docx"))
        doc.save(ruta_final)
        print(f"[DEBUG] DOCX generado: {ruta_final}")
        return True

    except Exception as e:
        print(f"[Error al crear DOCX] {e}")
        return False
...
def generar_informe_y_pdf(index, fila, usuario_id, empresa_id, origen_documento, nombre_documento_origen, reintentos=3, espera_base=2):
    try:
        ...
        if os.path.exists(ruta_docx):
            print(f"{nombre_archivo_docx} ya existe, será actualizado.")
            os.remove(ruta_docx)
        ...
        # Generar DOCX
        crear_docx(nombre_archivo_docx, datos_candidato, informe)
        print(f"DOCX creado: {nombre_archivo_docx}")

        # Actualizar mapa de PDFs
        mapa_actual = cargar_mapa_pdfs()
        mapa_actual[nombre_archivo_pdf] = nombre_candidato
        guardar_mapa_pdfs(mapa_actual)

        # Registrar ambos reportes
        dao_reportes.insertar_reporte({
        ...
        dao_reportes.insertar_reporte({
            'nombre_pdf': nombre_archivo_docx,
            'nombre_candidato': nombre_candidato,
            'tipo_reporte': 'individual_word',
            'nombre_documento_origen': nombre_documento_origen,
            'origen_documento': origen_documento,
            'empresa_id': empresa_id,
            'usuario_id': usuario_id
...
def procesar():
    ...
    archivo_postulacion = request.files.get('archivo_postulacion')
    archivo_pruebas = request.files.get('archivo_pruebas')
    ...
    # Procesar documento de postulación si se proporcionó
    if archivo_postulacion and archivo_postulacion.filename:
        nombre_postulacion = secure_filename(archivo_postulacion.filename)
        ruta_postulacion = os.path.join(app.config['UPLOAD_FOLDER'], nombre_postulacion)
        archivo_postulacion.save(ruta_postulacion)
        requisitos_extraidos = extraer_requisitos_propuesta_laboral(ruta_postulacion)
        print(f"[✓] Requisitos extraídos desde documento: {requisitos_extraidos}")
    ...
    with ProcessPoolExecutor(max_workers=8) as executor:
        future_to_index = {
            executor.submit(
                generador,
                idx, fila, usuario_id, empresa_id, origen_documento, nombre_documento_origen, requisitos_extraidos
...
def descargar_multiples():
    ...
    formato = request.form.get('formato', 'ambos')
    ...
    with zipfile.ZipFile(zip_buffer, 'w') as zipf:
        for nombre_pdf in seleccionados:
            ruta_pdf = os.path.join(app.config['PDF_FOLDER'], nombre_pdf)
            ruta_docx = ruta_pdf.replace('.pdf', '.docx')

            if formato in ['pdf', 'ambos'] and os.path.exists(ruta_pdf):
                zipf.write(ruta_pdf, arcname=os.path.basename(ruta_pdf))

            if formato in ['docx', 'ambos'] and os.path.exists(ruta_docx):
                zipf.write(ruta_docx, arcname=os.path.basename(ruta_docx))
...
def eliminar_multiples():
    ...
    for archivo in seleccionados:
        ruta_pdf = os.path.join(app.config['PDF_FOLDER'], archivo)
        archivo_docx = archivo.replace(".pdf", ".docx")
        ruta_docx = os.path.join(app.config['PDF_FOLDER'], archivo_docx)

        eliminado_pdf = False
        eliminado_docx = False
        ...
        # Eliminar gráfico asociado (opcional, por si lo tienes)
        ...
        # Eliminar de la base de datos si se eliminó PDF o DOCX
        if eliminado_pdf or eliminado_docx:
            eliminados.append(archivo)
            dao_reportes.eliminar_reporte_por_nombre_pdf(archivo)  # PDF
            dao_reportes.eliminar_reporte_por_nombre_pdf(archivo_docx)  # DOCX
        else:
...
@app.route('/descargar_docx/<nombre>')
def descargar_docx(nombre):
    docx_path = os.path.join(app.config['PDF_FOLDER'], nombre)
...
def nueva_empresa():
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            dao_usuario.insertar_admin_medio(nombre_admin, correo_admin, contrasena_admin, empresa_id, rol='admin_medio')
